...
class DjangoSemanticAnalyzer(BaseSemanticAnalyzer):
    ...
    def analyze(self, code: str, parsed_code: Dict) -> Dict[str, Any]:
        ...
        semantics = {
            ...
            'orm_subqueries': self._extract_subqueries(code),
            'orm_conditionals': self._extract_conditional_expressions(code),
            'orm_annotations': self._extract_annotations(code),
            'orm_aggregations': self._extract_aggregations(code),
            'orm_chains': self._extract_queryset_chains(code),
            'orm_optimizations': self._extract_orm_optimizations(code),
            'orm_q_objects': self._extract_q_objects(code),
            'orm_subqueries': self._extract_subqueries(code),
            'orm_conditionals': self._extract_conditional_expressions(code),
            'orm_annotations': self._extract_annotations(code),
            'orm_aggregations': self._extract_aggregations(code),
    ...
    # ===== Enhanced ORM Pattern Extractors =====
    
    def _extract_subqueries(self, code: str) -> List[Dict]:
        """Extract Django Subquery patterns"""
        subqueries = []
        
        # Pattern for Subquery usage
        subquery_patterns = [
            r'Subquery\s*\(([^)]+)\)',  # Subquery(...)
            r'OuterRef\s*\(\s*[\'"](\w+)[\'"]\s*\)',  # OuterRef('field')
            r'OuterRef\s*\(\s*(\w+)\s*\)',  # OuterRef(field)
        ]
        ...
        # Look for correlated subquery patterns
        correlated_pattern = r'Subquery\s*\([^)]*OuterRef[^)]*\)'
        for match in re.finditer(correlated_pattern, code, re.DOTALL):
            subqueries.append({
    ...
    def _extract_conditional_expressions(self, code: str) -> List[Dict]:
        """Extract Django conditional expressions (Case/When)"""
        conditionals = []
        
        # Pattern for Case/When expressions
        case_pattern = r'Case\s*\(([^)]*?When[^)]*)\)'
        for match in re.finditer(case_pattern, code, re.DOTALL):
            case_content = match.group(1)
            
            # Extract When clauses
            when_pattern = r'When\s*\(([^)]+)\)'
            when_clauses = []
            for when_match in re.finditer(when_pattern, case_content):
                when_clauses.append(when_match.group(1))
            
            conditionals.append({
                'type': 'Case',
                'when_clauses': when_clauses,
                'count': len(when_clauses),
                'line': code[:match.start()].count('\n') + 1
            })
        
        # Pattern for Coalesce
        coalesce_pattern = r'Coalesce\s*\(([^)]+)\)'
        for match in re.finditer(coalesce_pattern, code):
            conditionals.append({
                'type': 'Coalesce',
                'args': match.group(1),
                'line': code[:match.start()].count('\n') + 1
    ...
    def _extract_annotations(self, code: str) -> List[Dict]:
        """Extract Django annotate() patterns with detailed analysis"""
        annotations = []
        
        # Find annotate calls
        annotate_pattern = r'\.annotate\s*\(([^)]+)\)'
        for match in re.finditer(annotate_pattern, code, re.DOTALL):
            annotate_content = match.group(1)
            
            # Extract annotation fields
            field_patterns = [
                r'(\w+)\s*=\s*F\s*\(\s*[\'"]([^\'"]+)[\'"]\s*\)',  # field=F('field_name')
                r'(\w+)\s*=\s*F\s*\(\s*([^)]+)\)',  # field=F(field_name)
                r'(\w+)\s*=\s*([^,]+)',  # field=expression
            ]
            
            fields = []
            for pattern in field_patterns:
                for field_match in re.finditer(pattern, annotate_content):
                    ...
                    elif 'Coalesce' in expression:
                        expr_type = 'coalesce'
                    elif 'F(' in expression:
                        expr_type = 'field_reference'
                    elif 'Value(' in expression:
                        expr_type = 'value'
                    elif 'Subquery' in expression:
                        expr_type = 'subquery'
                    elif any(op in expression for op in ['/', '*', '+', '-']):
                        expr_type = 'calculation'
    ...
    def _extract_aggregations(self, code: str) -> List[Dict]:
        """Extract Django aggregation patterns"""
        aggregations = []
        
        # Pattern for Count with filter
        count_filter_pattern = r'Count\s*\(\s*[\'"](\w+)[\'"]\s*,\s*filter\s*=\s*Q\s*\(([^)]+)\)'
        for match in re.finditer(count_filter_pattern, code, re.DOTALL):
            field, filter_expr = match.groups()
            aggregations.append({
        ...
        # General aggregation patterns
        agg_patterns = [
            (r'Count\s*\(\s*[\'"](\w+)[\'"]\s*\)', 'Count'),
            (r'Sum\s*\(\s*[\'"](\w+)[\'"]\s*\)', 'Sum'),
            (r'Avg\s*\(\s*[\'"](\w+)[\'"]\s*\)', 'Avg'),
            (r'Min\s*\(\s*[\'"](\w+)[\'"]\s*\)', 'Min'),
            (r'Max\s*\(\s*[\'"](\w+)[\'"]\s*\)', 'Max'),
        ...
        # Aggregate method calls (not annotations)
        aggregate_pattern = r'\.aggregate\s*\(([^)]+)\)'
        for match in re.finditer(aggregate_pattern, code, re.DOTALL):
            aggregations.append({
                'type': 'aggregate_method',
    ...
    def _extract_queryset_chains(self, code: str) -> List[Dict]:
        """Extract and analyze QuerySet method chains"""
        chains = []
        
        # Pattern for chained queryset operations
        chain_pattern = r'(\.\w+\([^)]*\))(\.\w+\([^)]*\))+'
        ...
        for match in re.finditer(chain_pattern, code):
            ...
            for method_match in re.finditer(method_pattern, chain_text):
                method_name, method_args = method_match.groups()
                methods.append({
                    'name': method_name,
            ...
            if len(methods) >= 2:  # Only consider actual chains
                chains.append({
                    'methods': methods,
    ...
    def _extract_orm_optimizations(self, code: str) -> List[str]:
        """Extract ORM optimization patterns"""
        optimizations = []
        
        optimization_patterns = [
            (r'\.select_related\s*\(', 'select_related'),
            (r'\.prefetch_related\s*\(', 'prefetch_related'),
            (r'\.only\s*\(', 'only'),
            (r'\.defer\s*\(', 'defer'),
            (r'\.using\s*\(', 'using'),
            (r'\.iterator\s*\(', 'iterator'),
            (r'\.explain\s*\(', 'explain'),
            (r'\.raw\s*\(', 'raw'),
        ]
        
        for pattern, opt_type in optimization_patterns:
            if re.search(pattern, code):
                optimizations.append(opt_type)
        
        # Check for specific patterns
        if re.search(r'\.annotate\s*\(.*?\.filter\s*\(', code, re.DOTALL):
            optimizations.append('annotation_before_filter')
        
        if re.search(r'\.filter\s*\(.*?\.annotate\s*\(', code, re.DOTALL):
            optimizations.append('filter_before_annotation')
        
        return optimizations
    
    def _extract_q_objects(self, code: str) -> List[Dict]:
        """Extract Q object usage patterns"""
        q_objects = []
        
        # Simple Q object usage
        q_pattern = r'Q\s*\(([^)]+)\)'
        for match in re.finditer(q_pattern, code):
            q_content = match.group(1)
            
            # Check for logical operators
            has_or = '|' in q_content
            has_and = '&' in q_content
            has_not = '~' in q_content
            
            q_objects.append({
                'expression': q_content,
                'has_or': has_or,
                'has_and': has_and,
                'has_not': has_not,
                'complex': has_or or has_and or has_not,
                'line': code[:match.start()].count('\n') + 1
            })
        
        return q_objects
    
    def _calculate_orm_complexity(self, code: str) -> Dict[str, Any]:
        """Calculate ORM query complexity score"""
        subqueries = self._extract_subqueries(code)
        conditionals = self._extract_conditional_expressions(code)
        annotations = self._extract_annotations(code)
        aggregations = self._extract_aggregations(code)
        chains = self._extract_queryset_chains(code)
        
        # Calculate complexity score
        complexity_score = 0
        complexity_score += len(subqueries) * 10
        complexity_score += len(conditionals) * 5
        complexity_score += len(annotations) * 3
        complexity_score += len(aggregations) * 2
        complexity_score += len(chains) * 1
        
        # Check for advanced patterns
        has_correlated_subquery = any(s.get('type') == 'correlated_subquery' for s in subqueries)
        has_conditional_annotation = any(
            any(f['type'] == 'conditional' for f in a['fields'])
            for a in annotations
        )
        has_complex_q = any(q.get('complex') for q in self._extract_q_objects(code))
        
        return {
            'score': complexity_score,
            'has_correlated_subquery': has_correlated_subquery,
            'has_conditional_annotation': has_conditional_annotation,
            'has_complex_q': has_complex_q,
...
class ReactSemanticAnalyzer(BaseSemanticAnalyzer):
    ...
    def analyze(self, code: str, parsed_code: Dict) -> Dict[str, Any]:
        ...
        semantics = {
            ...
            'context_patterns': self._extract_context_patterns(code),
    ...
    def _extract_context_patterns(self, code: str) -> Dict[str, Any]:
        """Extract React Context API patterns specifically for Context + useReducer"""
        patterns = {
        ...
        context_pattern = r'(\w+)\s*=\s*(?:React\.)?createContext\s*\(([^)]*)\)'
        for match in re.finditer(context_pattern, code):
            context_name, default_value = match.groups()
            patterns['create_context_calls'].append({
                'name': context_name,
                'default_value': default_value.strip(),
                'line': code[:match.start()].count('\n') + 1
            })
        
        # Extract Context.Provider usage
        provider_pattern = r'<(\w+)\.Provider\s+[^>]*value\s*=\s*{([^}]+)}[^>]*>'
        for match in re.finditer(provider_pattern, code):
            context_name, value = match.groups()
            patterns['context_providers'].append({
                'context': context_name,
                'value_expression': value.strip(),
                'line': code[:match.start()].count('\n') + 1
            })
        
        # Extract custom hooks (specifically context hooks)
        custom_hook_pattern = r'const\s+(\w+)\s*=\s*\([^)]*\)\s*=>\s*{[^}]*useContext\s*\(([^)]+)\)'
        for match in re.finditer(custom_hook_pattern, code):
            hook_name, context_ref = match.groups()
            patterns['custom_hooks'].append({
                'name': hook_name,
                'context': context_ref.strip(),
                'type': 'context_hook',
                'line': code[:match.start()].count('\n') + 1
            })
        
        # Extract reducer patterns
        reducer_pattern = r'const\s+(\w+)\s*=\s*\([^)]*\)\s*=>\s*{[^}]*switch\s*\([^)]+\)'
        for match in re.finditer(reducer_pattern, code, re.DOTALL):
            patterns['reducer_patterns'].append({
                'name': match.group(1),
                'has_switch': True,
                'line': code[:match.start()].count('\n') + 1
            })
        ...
        action_type_pattern = r'(?:const\s+)?(\w+)\s*=\s*[\'"]([^\'"]+)[\'"]'
        for match in re.finditer(action_type_pattern, code):
            if any(action in match.group(0) for action in ['ADD', 'REMOVE', 'UPDATE', 'CLEAR']):
                patterns['action_types'].append({
                    'name': match.group(1),
                    'value': match.group(2)
                })
        
        # Extract dispatch usage
        dispatch_pattern = r'dispatch\s*\(\s*{\s*type\s*:\s*[\'"]([^\'"]+)[\'"]'
        for match in re.finditer(dispatch_pattern, code):
            patterns['dispatch_usage'].append({
...
class EnhancedIntermediateValidator(BaseValidator):
    ...
    def _validate_django_pattern(self, pattern_type: str, pattern: Dict, semantics: Dict, code: str) -> Dict:
        ...
        if pattern_type in [
            'subquery_usage', 'annotation_pattern', 'aggregation_pattern',
            'conditional_expression', 'queryset_optimization', 'q_object_usage',
            'complex_orm_pattern'
        ]:
            return self._validate_orm_pattern(pattern, semantics, code)        
        if pattern_type == 'model_field':
    ...
    def _validate_orm_pattern(self, pattern: Dict, semantics: Dict, code: str) -> Dict:
        """Main ORM pattern validation dispatcher"""
        pattern_type = pattern.get('type')
        
        if pattern_type == 'subquery_usage':
            return self._validate_orm_subquery(pattern, semantics)
        elif pattern_type == 'annotation_pattern':
            return self._validate_orm_annotation(pattern, semantics)
        elif pattern_type == 'aggregation_pattern':
            return self._validate_orm_aggregation(pattern, semantics)
        elif pattern_type == 'conditional_expression':
            return self._validate_orm_conditional(pattern, semantics)
        elif pattern_type == 'queryset_optimization':
            return self._validate_orm_optimization(pattern, semantics)
        elif pattern_type == 'q_object_usage':
            return self._validate_orm_q_object(pattern, semantics)
        elif pattern_type == 'complex_orm_pattern':
            return self._validate_complex_orm_pattern(pattern, semantics)
        else:
            return self._validate_generic_pattern(pattern_type, pattern, semantics, code)
    
    def _validate_orm_subquery(self, pattern: Dict, semantics: Dict) -> Dict:
        """Validate ORM subquery patterns"""
        subqueries = semantics.get('orm_subqueries', [])
        pattern_requirements = pattern.get('requirements', {})
        
        if not subqueries:
            return {'passed': False, 'message': "No subqueries found"}
        
        # Check for Subquery usage
        has_subquery = any('Subquery' in s.get('pattern', '') for s in subqueries)
        if pattern_requirements.get('requires_subquery') and not has_subquery:
            return {'passed': False, 'message': "Subquery not found"}
        
        # Check for OuterRef usage (correlated subqueries)
        has_outer_ref = any('OuterRef' in s.get('pattern', '') for s in subqueries)
        if pattern_requirements.get('requires_outer_ref') and not has_outer_ref:
            return {'passed': False, 'message': "OuterRef not found in subquery"}
        
        # Check for correlated subqueries
        has_correlated = any(s.get('type') == 'correlated_subquery' for s in subqueries)
        if pattern_requirements.get('requires_correlated') and not has_correlated:
            return {'passed': False, 'message': "Correlated subquery not found"}
        
        details = [f"Subqueries found: {len(subqueries)}"]
        if has_outer_ref:
            details.append("OuterRef used")
        if has_correlated:
            details.append("Correlated subquery detected")
        
        return {'passed': True, 'message': f"Subquery requirements met ({'; '.join(details)})"}
    
    def _validate_orm_annotation(self, pattern: Dict, semantics: Dict) -> Dict:
        """Validate ORM annotation patterns"""
        annotations = semantics.get('orm_annotations', [])
        pattern_requirements = pattern.get('requirements', {})
        
        if not annotations:
            return {'passed': False, 'message': "No annotations found"}
        
        required_fields = pattern_requirements.get('required_fields', [])
        field_types = pattern_requirements.get('field_types', {})
        
        all_fields = []
        for annotation in annotations:
            all_fields.extend(annotation.get('fields', []))
        
        # Check for required fields
        missing_fields = []
        for req_field in required_fields:
            if not any(f['field_name'] == req_field for f in all_fields):
                missing_fields.append(req_field)
        
        if missing_fields:
            return {'passed': False, 'message': f"Missing annotation fields: {', '.join(missing_fields)}"}
        
        # Check for field types
        for field_name, expected_type in field_types.items():
            matching_fields = [f for f in all_fields if f['field_name'] == field_name]
            if not matching_fields:
                return {'passed': False, 'message': f"Field '{field_name}' not found"}
            
            actual_type = matching_fields[0].get('type', 'unknown')
            if expected_type != actual_type:
                return {'passed': False, 'message': f"Field '{field_name}' has type '{actual_type}', expected '{expected_type}'"}
        
        # Check for specific expression patterns
        if pattern_requirements.get('requires_calculation'):
            has_calculation = any(f.get('type') == 'calculation' for f in all_fields)
            if not has_calculation:
                return {'passed': False, 'message': "No calculation annotations found"}
        
        if pattern_requirements.get('requires_conditional'):
            has_conditional = any(f.get('type') == 'conditional' for f in all_fields)
            if not has_conditional:
                return {'passed': False, 'message': "No conditional annotations found"}
        
        return {'passed': True, 'message': f"Annotations found ({len(annotations)} with {len(all_fields)} total fields)"}
    
    def _validate_orm_aggregation(self, pattern: Dict, semantics: Dict) -> Dict:
        """Validate ORM aggregation patterns"""
        aggregations = semantics.get('orm_aggregations', [])
        pattern_requirements = pattern.get('requirements', {})
        
        if not aggregations:
            return {'passed': False, 'message': "No aggregations found"}
        
        # Check for specific aggregation types
        required_types = pattern_requirements.get('required_types', [])
        missing_types = []
        
        for req_type in required_types:
            if req_type == 'Count_with_filter':
                has_type = any(a.get('type') == 'Count_with_filter' for a in aggregations)
            else:
                has_type = any(a.get('type') == req_type for a in aggregations)
            
            if not has_type:
                missing_types.append(req_type)
        
        if missing_types:
            return {'passed': False, 'message': f"Missing aggregation types: {', '.join(missing_types)}"}
        
        # Check for specific field aggregations
        required_field_aggs = pattern_requirements.get('required_field_aggregations', {})
        for field, agg_type in required_field_aggs.items():
            matching = [a for a in aggregations if a.get('field') == field and a.get('type') == agg_type]
            if not matching:
                return {'passed': False, 'message': f"Aggregation '{agg_type}' for field '{field}' not found"}
        
        agg_types = list(set(a.get('type', 'unknown') for a in aggregations))
        return {'passed': True, 'message': f"Aggregations found: {', '.join(agg_types)}"}
    
    def _validate_orm_conditional(self, pattern: Dict, semantics: Dict) -> Dict:
        """Validate ORM conditional expression patterns"""
        conditionals = semantics.get('orm_conditionals', [])
        pattern_requirements = pattern.get('requirements', {})
        
        if not conditionals:
            return {'passed': False, 'message': "No conditional expressions found"}
        
        # Check for Case/When
        has_case = any(c.get('type') == 'Case' for c in conditionals)
        if pattern_requirements.get('requires_case_when') and not has_case:
            return {'passed': False, 'message': "Case/When expressions not found"}
        
        # Check for Coalesce
        has_coalesce = any(c.get('type') == 'Coalesce' for c in conditionals)
        if pattern_requirements.get('requires_coalesce') and not has_coalesce:
            return {'passed': False, 'message': "Coalesce expressions not found"}
        
        # Check for Value
        has_value = any(c.get('type') == 'Value' for c in conditionals)
        if pattern_requirements.get('requires_value') and not has_value:
            return {'passed': False, 'message': "Value expressions not found"}
        
        # Check for division protection pattern
        if pattern_requirements.get('requires_division_protection'):
            # Look for Case/When patterns that protect against division by zero
            case_expressions = [c for c in conditionals if c.get('type') == 'Case']
            has_division_protection = False
            
            for case in case_expressions:
                when_clauses = case.get('when_clauses', [])
                for clause in when_clauses:
                    if 'avg_daily_sales__gt=0' in clause or 'then=F' in clause and '/' in clause:
                        has_division_protection = True
                        break
            
            if not has_division_protection:
                return {'passed': False, 'message': "Division by zero protection not found"}
        
        cond_types = list(set(c.get('type', 'unknown') for c in conditionals))
        return {'passed': True, 'message': f"Conditional expressions: {', '.join(cond_types)}"}
    
    def _validate_orm_optimization(self, pattern: Dict, semantics: Dict) -> Dict:
        """Validate ORM optimization patterns"""
        optimizations = semantics.get('orm_optimizations', [])
        pattern_requirements = pattern.get('requirements', {})
        
        # Check for specific optimizations
        required_optimizations = pattern_requirements.get('required_optimizations', [])
        missing_optimizations = []
        
        for req_opt in required_optimizations:
            if req_opt not in optimizations:
                missing_optimizations.append(req_opt)
        
        if missing_optimizations:
            return {'passed': False, 'message': f"Missing optimizations: {', '.join(missing_optimizations)}"}
        
        # Check optimization order patterns
        if pattern_requirements.get('requires_select_related'):
            if 'select_related' not in optimizations:
                return {'passed': False, 'message': "select_related not used"}
        
        if pattern_requirements.get('requires_prefetch_related'):
            if 'prefetch_related' not in optimizations:
                return {'passed': False, 'message': "prefetch_related not used"}
        
        if optimizations:
            return {'passed': True, 'message': f"ORM optimizations: {', '.join(optimizations)}"}
        else:
            return {'passed': False, 'message': "No ORM optimizations found"}
    
    def _validate_orm_q_object(self, pattern: Dict, semantics: Dict) -> Dict:
        """Validate Q object usage patterns"""
        q_objects = semantics.get('orm_q_objects', [])
        pattern_requirements = pattern.get('requirements', {})
        
        if not q_objects:
            return {'passed': False, 'message': "No Q objects found"}
        
        # Check for complex Q objects (with | or &)
        has_complex_q = any(q.get('complex') for q in q_objects)
        if pattern_requirements.get('requires_complex_q') and not has_complex_q:
            return {'passed': False, 'message': "Complex Q object (with | or &) not found"}
        
        # Check for OR conditions (|)
        has_or = any(q.get('has_or') for q in q_objects)
        if pattern_requirements.get('requires_or_condition') and not has_or:
            return {'passed': False, 'message': "Q object with OR condition (|) not found"}
        
        q_count = len(q_objects)
        complex_count = sum(1 for q in q_objects if q.get('complex'))
        
        return {'passed': True, 'message': f"Q objects found ({q_count} total, {complex_count} complex)"}
    
    def _validate_complex_orm_pattern(self, pattern: Dict, semantics: Dict) -> Dict:
        """Validate complex ORM patterns from the problem requirements"""
        complexity = semantics.get('orm_complexity', {})
        pattern_requirements = pattern.get('requirements', {})
        
        # Check for specific complexity requirements
        min_complexity = pattern_requirements.get('min_complexity_score', 0)
        actual_score = complexity.get('score', 0)
        
        if actual_score < min_complexity:
            return {'passed': False, 'message': f"ORM complexity too low (score: {actual_score}, required: {min_complexity})"}
        
        # Check for specific patterns
        requires_correlated = pattern_requirements.get('requires_correlated_subquery', False)
        if requires_correlated and not complexity.get('has_correlated_subquery'):
            return {'passed': False, 'message': "Correlated subquery required but not found"}
        
        requires_conditional_annotation = pattern_requirements.get('requires_conditional_annotation', False)
        if requires_conditional_annotation and not complexity.get('has_conditional_annotation'):
            return {'passed': False, 'message': "Conditional annotation required but not found"}
        
        # Build success message with details
        details = []
        if complexity.get('subquery_count', 0) > 0:
            details.append(f"{complexity['subquery_count']} subqueries")
        if complexity.get('annotation_count', 0) > 0:
            details.append(f"{complexity['annotation_count']} annotations")
        if complexity.get('aggregation_count', 0) > 0:
            details.append(f"{complexity['aggregation_count']} aggregations")
        
        if details:
            return {'passed': True, 'message': f"Complex ORM pattern satisfied ({'; '.join(details)})"}
        else:
            return {'passed': True, 'message': "Basic ORM patterns found"}
    ...
    # Add to the _validate_react_pattern method in EnhancedIntermediateValidator

    def _validate_context_pattern(self, pattern: Dict, semantics: Dict, code: str) -> Dict:
        """Validate React Context API patterns"""
        pattern_type = pattern.get('context_type')
        component_name = pattern.get('component_name')
        hook_name = pattern.get('hook_name')
        
        context_patterns = semantics.get('context_patterns', {})
        
        if pattern_type == 'provider':
            providers = context_patterns.get('context_providers', [])
            if component_name:
                matching = [p for p in providers if p['context'].replace('.Provider', '') == component_name.replace('Context', '')]
                if matching:
                    return {'passed': True, 'message': f"{component_name}.Provider found with value prop"}
            elif providers:
                return {'passed': True, 'message': f"Context provider found: {providers[0]['context']}"}
            return {'passed': False, 'message': f"Context provider not found for {component_name}"}
        
        elif pattern_type == 'use_context':
            hooks = context_patterns.get('custom_hooks', [])
            if hook_name:
                matching = [h for h in hooks if h['name'] == hook_name]
                if matching:
                    return {'passed': True, 'message': f"Custom hook '{hook_name}' using useContext"}
            elif hooks:
                hook_names = [h['name'] for h in hooks]
                return {'passed': True, 'message': f"Custom hooks found: {', '.join(hook_names)}"}
            return {'passed': False, 'message': f"Custom hook '{hook_name}' not found"}
        
        return {'passed': False, 'message': f"Unknown context pattern: {pattern_type}"}

    def _validate_custom_hook_pattern(self, pattern: Dict, semantics: Dict, code: str) -> Dict:
        """Validate custom hook patterns"""
        hook_name = pattern.get('hook_name')
        must_use_react_hooks = pattern.get('must_use_react_hooks', False)
        must_return = pattern.get('must_return', False)
        
        # Get hooks from different sources
        custom_hooks = semantics.get('custom_hooks', [])
        context_patterns = semantics.get('context_patterns', {})
        
        # Safely get context_hooks
        context_hooks = []
        if context_patterns and isinstance(context_patterns, dict):
            context_hooks = context_patterns.get('custom_hooks', [])
        
        # Combine both hook sources
        all_hooks = []
        if custom_hooks:
            all_hooks.extend(custom_hooks)
        if context_hooks:
            all_hooks.extend(context_hooks)
        
        if not all_hooks:
            return {'passed': False, 'message': "No custom hooks found"}
        
        if hook_name:
            # Safely check for matching hooks
            matching = []
            for h in all_hooks:
                if isinstance(h, dict) and h.get('name') == hook_name:
                    matching.append(h)
            
            if not matching:
                return {'passed': False, 'message': f"Custom hook '{hook_name}' not found"}
            
            hook_info = matching[0]
            
            # Check if it uses React hooks
            if must_use_react_hooks:
                # Define a simple helper function here
                def check_hook_uses_react_hooks(hook_name: str, code: str) -> bool:
                    """Check if a custom hook uses React hooks"""
                    # Find the hook definition
                    hook_pattern = rf'const\s+{hook_name}\s*=\s*\([^)]*\)\s*=>\s*{{([^}}]+)}}'
                    match = re.search(hook_pattern, code, re.DOTALL)
                    
                    if match:
                        hook_body = match.group(1)
                        # Check for React hook usage in the body
                        react_hooks = ['useState', 'useEffect', 'useContext', 'useReducer', 'useCallback', 'useMemo', 'useRef']
                        return any(hook in hook_body for hook in react_hooks)
                    
                    return False
                
                uses_hooks = check_hook_uses_react_hooks(hook_name, code)
                if not uses_hooks:
                    return {'passed': False, 'message': f"Custom hook '{hook_name}' doesn't use React hooks"}
            
            # Check if it returns a value (optional)
            if must_return:
                # Simple check for return statement in hook
                if f'const {hook_name} =' in code and 'return' in code[code.find(f'const {hook_name} ='):]:
                    return {'passed': True, 'message': f"Custom hook '{hook_name}' found and returns value"}
                else:
                    return {'passed': True, 'message': f"Custom hook '{hook_name}' found (return check skipped)"}
            
            return {'passed': True, 'message': f"Custom hook '{hook_name}' found"}
        ...
        if hook_names:
            ...
            return {'passed': True, 'message': "Custom hooks detected"}
